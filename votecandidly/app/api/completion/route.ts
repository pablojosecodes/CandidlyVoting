...
export async function POST(req: Request) {
  ...
  const response = await anthropic.completions.create({
    ...
    INSTRUCTION: Respond (with a short answer) to the human's prompt asking about the upcoming district 11 Caliornia senate race  (in which SCott and Yvette are competing) or the candidates. If they ask about ANYTHING else, return "That is out of scope.
